...
프로세스는 실행중인 프로그램을 의미한다. 운영체제는 프로그램을 필요한 자원과 하드웨어와의 I/O를 통해 실행시켜주는데 프로세스는 이런 하나의 작업을 나타낸다.

프로세스가 되기 위해서는 아래와 같은 자원이 필요하게 된다.
...
* Ready
  * 대기하고 있다 바로 Running상태가 아니라 Running이 될 수 있도록 대기
* Terminated
  * 모든 실행이 완료되어 종료된 상태

**PCB\(Process Control Block\) or TCB\(Task control Block\)**

각 프로세스가 유지해야할 정보를 `PCB`에 저장하고 관리, 그 중 중요한 요소는 다음과 같다.
...
따라서 위 정보를 알고 있다면 해당 프로세스의 다음 수행할 명령어와 그전의 상태를 가지고 다음 작업을 수행하며 여러개의 프로세스가 병렬적으로 수행되어도 각각의 프로세스는 마치 하나의 줄기가 뻗어나가는 것 처럼 작업을 이어서 수행할 수 있게 된다. 하지만 프로세스가 유지해야할 모든 정보를 가지고 있기 때문에 굉장히 무거운 구조체 데이터이다.
